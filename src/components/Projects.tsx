...
const projects: Project[] = [
  {
    ...
    description: "An AI-driven educational assistant that uses Large Language Models to provide personalized learning experiences and answer complex questions across various subjects.",
  ...
  },
  {
    ...
    description: "A mobile application using computer vision to assist visually impaired individuals in navigating their environment and recognizing objects and text.",
...
const ProjectCard: React.FC<Project> = ({ title, description, technologies, role, github, demo, image }) => (
  <motion.div
    ...
    className="bg-gray-800 rounded-lg shadow-md overflow-hidden border border-gold-500"
  >
    <img src={image} alt={title} className="w-full h-48 object-cover" />
    <div className="p-6">
      <h3 className="text-xl font-bold mb-2 text-gold-500">{title}</h3>
      <p className="mb-4 text-gray-300">{description}</p>
      <p className="mb-2 text-gray-300"><strong className="text-gold-300">Role:</strong> {role}</p>
      <p className="mb-4 text-gray-300"><strong className="text-gold-300">Technologies:</strong> {technologies.join(", ")}</p>
      <div className="flex space-x-4">
        {github && (
          <a href={github} target="_blank" rel="noopener noreferrer" className="flex items-center text-gold-500 hover:text-gold-300 transition-colors duration-200">
            <Github className="w-5 h-5 mr-1" /> GitHub
          </a>
        )}
        {demo && (
          <a href={demo} target="_blank" rel="noopener noreferrer" className="flex items-center text-gold-500 hover:text-gold-300 transition-colors duration-200">
    ...
    <section id="projects" className="py-16 bg-gray-900">
      <div className="container mx-auto px-4">
        <motion.h2
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          className="text-3xl font-bold mb-8 text-gold-500"
        >
          Projects
        </motion.h2>
